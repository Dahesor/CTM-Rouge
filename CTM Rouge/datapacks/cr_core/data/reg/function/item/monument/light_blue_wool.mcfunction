data modify storage reg:item new set value {\
    id:"light_blue_wool",\
    rarity:"mythic",\
    base_price:640,\
    lore_line_count:4,\
    in_shop:false,\
    spawner_loot:false,\
    stack:{\
        id:"light_blue_wool",\
        components:{\
        max_stack_size:1,\
        custom_data:{item:{class:"wool",id:"light_blue_wool"}},\
        enchantments:{binding_curse:1},\
        equippable:{\
            slot:"head",allowed_entities:"player",equip_on_interact:false,\
            equip_sound:"intentionally_empty",dispensable:false,can_be_sheared:false,\
            damage_on_hurt:false,swappable:false},\
        "minecraft:tooltip_display":{hidden_components:["enchantments"]},\
        damage_resistant:{types:"#rcore:all"},"minecraft:tooltip_style":"crouge:wool/light_blue",\
        item_model:"crouge:wools/light_blue"\
        }\
    }\
}

scoreboard players set wool.light_blue.score const 220

function reg:item/reg/new