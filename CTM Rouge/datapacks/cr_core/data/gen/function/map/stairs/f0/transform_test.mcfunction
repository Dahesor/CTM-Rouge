execute unless score floor.level options matches 0 run return fail

data modify storage ram: map set value []
data modify storage ram: map append from storage map: construct.grid.cells[][{connections:{pos_x:true,pos_z:false,neg_x:false,neg_z:false},type:"room"}]
data modify storage ram: map append from storage map: construct.grid.cells[][{connections:{pos_x:false,pos_z:true,neg_x:false,neg_z:false},type:"room"}]
data modify storage ram: map append from storage map: construct.grid.cells[][{connections:{pos_x:false,pos_z:false,neg_x:true,neg_z:false},type:"room"}]
data modify storage ram: map append from storage map: construct.grid.cells[][{connections:{pos_x:false,pos_z:false,neg_x:false,neg_z:true},type:"room"}]
