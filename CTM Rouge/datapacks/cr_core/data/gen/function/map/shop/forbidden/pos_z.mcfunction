...
execute store result storage ram: disjoint.forbidden_edges[-1].from int 1 run scoreboard players get $this calculator.cr
execute store result storage ram: disjoint.forbidden_edges[-1].to int 1 run scoreboard players operation $this calculator.cr += floor.side_length options