...
execute store result score #rand calculator.cr run random value 0..2 gen:unique_location

execute if score #rand calculator.cr >= #f0_mid_taken calculator.cr run scoreboard players add #rand calculator.cr 1
...
execute unless data storage ram: map_cell{type:"room"} run return run execute if score #try calculator.cr matches 1.. run function gen:map/uniques/f0/white_wool

data modify storage ram: map_cell merge value {type:"special",special:"white_wool"}
