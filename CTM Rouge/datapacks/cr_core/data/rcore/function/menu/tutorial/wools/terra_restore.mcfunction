...
data modify storage ram: text set value "清理一个房间的所有刷怪笼掉落"
data modify storage run: color set value "#e8cc13"


function reg:item/get/prepare
data modify storage ram: item set from entity 0-0-0-0-0 Items[{Slot:0b}]
data modify storage ram: dialog.dialog set value {type:"notice",title:"羊毛说明",body:[{type:"plain_message",contents:{text:"",font:"crouge:title",extra:[""]}},{type:"item",item:{id:"structure_void"}},{type:"plain_message",contents:""},{type:"plain_message",contents:""},{type:"plain_message",contents:[""]}],action:{label:{translate:"gui.done"}},pause:false}
...
data modify storage ram: dialog.dialog.body[2].contents set from storage ram: item.components."minecraft:lore"[-1]
