...
execute store result score $this team run data get storage team: data[0].team_id

function rcore:stream/start/spawn_point/__spawn_players with storage run: i
scoreboard players add #count calculator.cr 1
execute if score #count calculator.cr >= game.team_count options run return 1
function rcore:math/team_list/rotate
function rcore:stream/start/spawn_point/_spawn_team