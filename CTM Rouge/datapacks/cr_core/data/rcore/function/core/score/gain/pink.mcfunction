...
## Broadcast
    ...
    data modify storage ram: broadcast.self set value [{text:"[✔] 您带回了",color:"green"},{translate:"item.pink_wool"},"，队伍分增加",{score:{name:"wool.pink.score",objective:"const"},color:"yellow"}]
...
execute if score @s team matches 1 run scoreboard players operation score.red data += wool.pink.score const
execute if score @s team matches 2 run scoreboard players operation score.blue data += wool.pink.score const
execute if score @s team matches 3 run scoreboard players operation score.yellow data += wool.pink.score const
execute if score @s team matches 4 run scoreboard players operation score.green data += wool.pink.score const
