...
execute as @a[scores={join_game=1..},distance=..100] if score @s team = $this team if score @s room = $this room run title @s[tag=!has_wool] actionbar {translate:"actionbar.tp_hint",color:"yellow"}