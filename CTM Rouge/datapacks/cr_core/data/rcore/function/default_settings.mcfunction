scoreboard players set REVIVE_COOLDOWN options 600
scoreboard players set SCORE.TERRA_RESTORE options 20
scoreboard players set EMERALD.TERRA_RESTORE options 5
scoreboard players set EMERALD.PLAYER_BOOST options 25
scoreboard players set LOOT_DROP.PLAYER_BOOST options 30
scoreboard players set TP.CD options 100
scoreboard players set GameLength options 48000

scoreboard players set floor_0.x data 0
scoreboard players set floor_0.y data 20
scoreboard players set floor_0.z data 0
scoreboard players set floor_1.x data 1000
scoreboard players set floor_1.y data 20
scoreboard players set floor_1.z data 0
scoreboard players set floor_2.x data 0
scoreboard players set floor_2.y data 20
scoreboard players set floor_2.z data 1000

scoreboard players set diff.f0 options 100
scoreboard players set diff.f1 options 160
scoreboard players set diff.f2 options 230

scoreboard players set stair_1.open_time options 12000
scoreboard players set stair_2.open_time options 24000